...
def demo_noisy_sinusoid():
    ...
    A1 = 1#Amplitude 1
    A2 = 0.53#and 2
    signal = A1 * np.sin(2 * np.pi * f1 * t) + A2 * np.sin(2 * np.pi * f2 * t) + A1/A2 * np.sin(2 * np.pi * f2/f1 * t)
    
    # Add some noise
    noisy_signal = signal + 0.2 * np.random.randn(len(t))
    
    # Compute FT using our module
    transform = ft.dft(noisy_signal)

    # Create frequency grid
    half_n = len(transform) // 2
    magnitudes = np.abs(transform[:half_n]) / len(noisy_signal)
